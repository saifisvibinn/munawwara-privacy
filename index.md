...
- **Voice and video calls (Internet / VoIP):** Microphone (and camera when used for video) via our real-time communications provider; call-related metadata such as timestamps, duration, and connection status
...
- **Moderator messages:** Text messages, voice messages, and text-to-speech (TTS) announcements sent by moderators to the group or to individual pilgrims (pilgrims receive these messages; the App is not designed for pilgrims to send group chat messages)
- **Push notifications:** Firebase Cloud Messaging device tokens and notification delivery data

### E. Technical and usage data

- App version, language preference, and authentication/session tokens
...
- Enabling voice/video calls between pilgrims and moderators
...
**Important:** The App is a **group coordination and safety tool**. It does **not** replace emergency services. In a life-threatening emergency, contact local authorities and official Hajj/Umrah emergency services.
...
- **Service providers** (processors acting on our behalf), including:
  ...
  - **Agora** — voice/video (WebRTC) calls
  - **Google Cloud Platform** — application hosting, storage, scheduled tasks, text-to-speech, and translation services used for reminders and announcements
  - **MongoDB Atlas** — database hosting
  - **Email delivery** — password reset and account-related email (where applicable)
...
- When you **log out**, session tokens are cleared; some profile or location fields may remain until account deletion

**Your rights:** You may request access, correction, or deletion of your personal data by contacting your travel operator’s administrator or emailing **munawwaracare@gmail.com**. We will respond within a reasonable time subject to verification and applicable law.

---

## 7. Security

We use measures such as HTTPS for API traffic, secured WebSocket connections, and encrypted real-time media (WebRTC) where supported. No method of transmission or storage is 100% secure.

---

## 8. International transfers and hosting

Our application servers are hosted on **Google Cloud** (region configured for our deployment, e.g. European Union). Database and other providers may process data in regions configured in their respective services. By using the App, you understand that your information may be processed in those locations.
